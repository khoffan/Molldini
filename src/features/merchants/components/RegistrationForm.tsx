...
import type { Address } from '../../address/interface/addressInterface';
import type { CreateMerchantInput } from '../interfaces/merchantInterface';
...
function RegistrationForm({ userId }: { userId?: string }) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <section>
                <h2 className="text-lg font-semibold text-content mb-4 border-l-4 border-primary pl-3">
                    ข้อมูลร้านค้า
                </h2>
                <div className="grid grid-cols-1 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-content mb-1">ชื่อร้านค้า (Store Name)</label>
                        <input
                            {...register("name")}
                            className={`w-full p-2.5 border rounded-lg outline-none focus:ring-2 transition-all ${errors.name ? "border-red-500 focus:ring-red-100" : "border-border-main focus:ring-primary/20"
                                }`}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-content mb-1">รายละเอียดร้านค้า (เลือกได้)</label>
                        <textarea
                            {...register("description")}
                            rows={3}
                            className="w-full p-2.5 border border-border-main rounded-lg outline-none focus:ring-2 focus:ring-primary/20"
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-content mb-1">เลขที่บ้าน, ถนน, ซอย</label>
                        <input
                            {...register("detail")}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-content mb-1">ตำบล/แขวง</label>
                        <input {...register("subDistrict")} className="w-full p-2.5 border border-gray-300 rounded-lg" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-content mb-1">อำเภอ/เขต</label>
                        <input {...register("district")} className="w-full p-2.5 border border-gray-300 rounded-lg" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-content mb-1">จังหวัด</label>
                        <input {...register("province")} className="w-full p-2.5 border border-gray-300 rounded-lg" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-content mb-1">รหัสไปรษณีย์</label>
            ...
            <button
                ...
                className="w-full bg-primary text-white py-3 rounded-xl font-bold hover:bg-primary/90 active:scale-[0.98] transition-all disabled:bg-muted shadow-lg"
            >
