...
import type { Merchant } from '../interfaces/merchantInterface';
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-surface p-6 rounded-2xl shadow-sm border border-border-main flex items-center gap-4">
                    <div className="p-4 rounded-xl bg-primary-light">
                        <Package className="w-6 h-6 text-primary" />
                    </div>
                    <div>
                        <p className="text-sm text-muted font-medium">รายการสินค้า</p>
                        <h3 className="text-2xl font-bold text-content">{totalProducts} รายการ</h3>
                    </div>
                </div>
                ...
                <div className="bg-surface p-6 rounded-2xl shadow-sm border border-border-main flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-sm text-muted font-medium">สต็อกรวมทั้งหมด</p>
                        <h3 className="text-2xl font-bold text-content">{totalStock} ชิ้น</h3>
                    </div>
                </div>
                ...
                <div className="bg-surface p-6 rounded-2xl shadow-sm border border-border-main flex items-center gap-4">
                    ...
                    <div className="overflow-hidden">
                        <p className="text-sm text-muted font-medium">ที่ตั้งร้านค้า</p>
            ...
            <div className="bg-surface p-6 rounded-2xl shadow-sm border border-border-main">
                <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                    {merchant.logoUrl ? (
                        <img
                            src={merchant.logoUrl.url}
                            alt={merchant.name}
                            className="w-24 h-24 rounded-2xl object-cover border-2 border-border-main"
                        />
                    ) : (
                        <div className="w-24 h-24 rounded-2xl bg-surface-hover flex items-center justify-center border-2 border-dashed border-border-main">
                            <Globe className="w-8 h-8 text-muted" />
                        </div>
                    )}

                    <div className="flex-1">
                        <h2 className="text-2xl font-black text-content">{merchant.name}</h2>
                        <p className="text-muted mt-1">{merchant.description || 'ไม่มีคำอธิบายร้านค้า'}</p>
                        <div className="mt-3 flex flex-wrap gap-2">
                            <span className="text-xs bg-surface-hover px-2 py-1 rounded text-muted uppercase font-bold">
                                ID: {merchant.id.split('-')[0]}...
                            </span>
                            <span className="text-xs bg-primary-light px-2 py-1 rounded text-primary font-bold italic">
                                เป็นสมาชิกเมื่อ: {new Date(merchant.createdAt).toLocaleDateString('th-TH')}
                            </span>
                        </div>
                    </div>

                    <button onClick={() => navigate("/add-product")} className="w-full md:w-auto flex items-center justify-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-bold hover:bg-primary/90 transition-all shadow-lg">
            ...
            <div className="bg-surface rounded-2xl shadow-sm border border-border-main overflow-hidden">
                <div className="p-6 border-b border-border-main flex justify-between items-center bg-surface">
                    <h3 className="text-lg font-bold text-content">จัดการสินค้าภายในร้าน</h3>
                ...
                {merchant.products && merchant.products.length > 0 ? (
                    <>
                        ...
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-surface-hover/50 text-muted text-[11px] uppercase tracking-[0.1em] font-black">
                                    <tr>
                    ...
                    <div className="p-20 text-center">
                        ...
                        <p className="text-muted font-medium">ยังไม่มีสินค้าในร้านค้าของคุณ</p>
                    </div>
                )}
            </div>
        </div>
