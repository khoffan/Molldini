...
import type { Product, ProductInput } from "../interfaces/productInterface";
import { uploadToCloudinary } from "../../../common/lib/uploadfileToFirestoage";
...
export const fetchProducts = createAsyncThunk(
    ...
    async (id: string | undefined, { rejectWithValue }) => {
        try {
            ...
            if (id) return newData as Product;
    ...
    {
        // ⚡ ส่วนสำคัญ: Logic การตัดสินใจว่าจะรัน Thunk หรือไม่
        condition: (id, { getState }) => {
            const { product } = getState() as RootState;

            // ถ้ามีการระบุ ID มา ให้ดึงใหม่เสมอ (Fresh Detail Data)
            if (id) return true;

            // ถ้าเป็นหน้า List และกำลังโหลดอยู่ (ป้องกัน Request ซ้อน)
